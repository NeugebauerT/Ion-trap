...
"Calculating the fragmentation process in quadrupole ion traps" by T.S. Neugebauer
...
z0_text = "$z_0$"
zfrag_text = "$z_{\mathrm{frag}}$"
fRF_text = "$f_{\mathrm{RF}}$"
Teff_text = "$T_{\mathrm{eff}}$"
...
zmax_fragmentation_list = [[] for cutoff in LMCO_to_calculate]
...
for index_cutoff, cutoff in enumerate(LMCO_to_calculate):
    ...
    zmax_fragmentation_list[index_cutoff] = [0 for mass in precursormass_to_calculate]
    ...
    for index_precmass, precursormass in enumerate(precursormass_to_calculate):
        ...
        zmax_fragmentation_list[index_cutoff][index_precmass] = zmax_prec_fragmentation
                ...
                for cutoff in summed_Coulomb_factors.keys():
                    for precmass in summed_Coulomb_factors[cutoff].keys():
                        ...
                        for cfactor in summed_Coulomb_factors[cutoff][precmass].keys():
                            if cfactor == "masslist":
                                continue
                            print(f"Coulomb repulsion: {cfactor*100}%, ", file=savefile)
    ...
    for precursormass in summed_Coulomb_factors[cutoff].keys():
        ...
        for coulomb_factor in summed_Coulomb_factors[cutoff][precursormass].keys():
            ...
            if coulomb_factor != "masslist":
                ...
                legend.append(f"lost {coulomb_factor*100:.1f}")
                plt.plot(fragmassplot, coldplot, marker="o", markersize=4)
                legend.append(f"cold {coulomb_factor*100:.1f}")
        zmax_prec_fragmentation = zmax_fragmentation_list[LMCO_to_calculate.index(cutoff)][precursormass_to_calculate.index(precursormass)]
        plt.title(f"LMCO = {cutoff}%, Precursor m/z = {precursormass}/{charge_prec_z}, lost% and cold%\n"
...
for cutoff in summed_Coulomb_factors.keys():
    for precursormass in summed_Coulomb_factors[cutoff].keys():
        ...
        zmax_prec_fragmentation = zmax_fragmentation_list[LMCO_to_calculate.index(cutoff)][precursormass_to_calculate.index(precursormass)]
        plt.title(f"LMCO = {cutoff}%, Precursor m/z = {precursormass}/{charge_prec_z}, avgT\n"
                  f"{z0_text} = {z0:.2f} mm, {fRF_text} = {fRF} kHz, {zfrag_text} = {zmax_prec_fragmentation:.2f} mm\n"
                  f"{Teff_text} = {T_eff_prec_fragmentation} K")
        plt.xlabel("Fragment mass (m/z)")
    ...
    for precursormass in summed_Coulomb_factors[cutoff].keys():
        ...
        for coulomb_factor in summed_Coulomb_factors[cutoff][precursormass].keys():
            ...
            if coulomb_factor != "masslist":
                ...
                legend.append(f"{coulomb_factor*100:.1f}")
        zmax_prec_fragmentation = zmax_fragmentation_list[LMCO_to_calculate.index(cutoff)][precursormass_to_calculate.index(precursormass)]
        plt.title(f"LMCO = {cutoff}%, Precursor m/z = {precursormass}/{charge_prec_z}, trappedavgTplot\n"
                  f"{z0_text} = {z0:.2f} mm, {fRF_text} = {fRF} kHz, {zfrag_text} = {zmax_prec_fragmentation:.2f} mm\n"
                  f"{Teff_text} = {T_eff_prec_fragmentation} K")
        plt.xlabel("Fragment mass (m/z)")
